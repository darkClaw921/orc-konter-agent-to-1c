...
│   │   ├── schemas.py                 # Pydantic схемы для API (ContractUploadResponse, ContractStatusResponse, ContractDataResponse, ContractRawTextResponse)
│   │   ├── contract_schemas.py       # Pydantic схемы для валидации контрактов (ContractDataSchema с валидаторами validate_inn, validate_kpp, validate_legal_entity_type, validate_kpp_required, validate_dates, sync_locations; ResponsiblePerson, ServiceLocation)
...
│   │       └── endpoints/
│   │           ├── __init__.py
│   │           ├── contracts.py       # Эндпоинты для работы с контрактами (upload, status, data, list, delete)
...
│   │   ├── __init__.py
│   │   ├── document_processor.py     # Парсинг DOCX и PDF файлов с использованием docling для извлечения таблиц (DocumentProcessor: load_document, extract_text, extract_tables, get_tables_markdown, extract_sections, get_context_for_llm, split_into_chunks, get_chunks_for_llm, estimate_tokens, _extract_table_from_docling, _convert_table_to_markdown)
│   │   ├── document_validator.py     # Валидация загруженных документов DOCX и PDF (DocumentValidator)
│   │   ├── llm_service.py            # Интеграция с LLM провайдерами (LLMService: extract_contract_data, validate_extracted_data, aggregate_chunks_data, merge_extracted_data; BaseLLMProvider, OpenAIProvider, YandexGPTProvider)
│   │   ├── prompts.py                # Prompt templates для LLM (EXTRACT_CONTRACT_DATA_PROMPT, VALIDATE_EXTRACTED_DATA_PROMPT, MERGE_CHUNKS_DATA_PROMPT)
...
│   │   ├── __init__.py
│   │   ├── orchestrator.py           # Оркестрация обработки контракта (AgentOrchestrator)
│   │   └── state_manager.py          # Управление состоянием агента (StateManager, AgentState)
│   │
│   ├── tasks/                         # Celery задачи
│   │   ├── __init__.py
│   │   ├── celery_app.py             # Конфигурация Celery
│   │   └── processing_tasks.py       # Асинхронные задачи обработки (process_contract_task)
│   │
│   └── utils/                         # Утилиты
│       ├── __init__.py
...
3. **SGR Agent Core** (`app/agent/orchestrator.py`) - оркестрирует процесс обработки контракта:
   ...
   - Извлекает данные через `LLMService`
   - При обработке больших документов разбитых на чанки: накапливает полную информацию о договоре из предыдущих чанков в переменной `accumulated_data` и добавляет контекстный блок в начало каждого последующего чанка через метод `_build_chunk_context()`. Контекст включает: основную информацию о договоре (название, номер, дата, цена, НДС), описание услуг/товаров, услуги из спецификации/таблиц (название, количество, единица измерения, цена за единицу, общая стоимость), даты оказания услуг, условия оплаты, дополнительную информацию (порядок приема-сдачи, наличие спецификации, порядок ценообразования, формы отчетности, дополнительные условия, техническая информация), информацию о контрагентах (ИНН, наименование, роль, КПП), агентах и ответственных лицах (ФИО, должность, контакты), а также контактную информацию (телефоны, email, адреса). Метод `_build_chunk_context()` формирует структурированный контекстный блок со всеми накопленными данными для передачи в LLM при обработке каждого чанка
...
4. **Services** (`app/services/`) - реализуют бизнес-логику:
   - `DocumentProcessor` - парсит DOCX и PDF файлы с использованием docling для извлечения структурированных данных из таблиц, разбивает многостраничные документы на чанки (split_into_chunks, get_chunks_for_llm) для обработки через LLM. Таблицы извлекаются в markdown формате и включаются в контекст для LLM (extract_tables, get_tables_markdown, _extract_table_from_docling). Поддерживает fallback на python-docx для DOCX файлов при ошибках docling
   - `LLMService` - взаимодействует с LLM провайдерами, объединяет результаты извлечения из разных чанков через финальную агрегацию через LLM (aggregate_chunks_data) с разрешением конфликтов. При агрегации передает полный накопленный контекст из всех предыдущих чанков (accumulated_context) для каждого чанка и финальный накопленный контекст для лучшего разрешения конфликтов. При ошибке использует fallback на простое объединение (merge_extracted_data)
   - `prompts.py` - содержит промпты для LLM: EXTRACT_CONTRACT_DATA_PROMPT с явными инструкциями по извлечению полной контактной информации об агентах и контрагентах (телефоны, email, адреса, должности), а также услуг из спецификации/таблиц (название, количество, единица измерения, цена за единицу, общая стоимость, описание), VALIDATE_EXTRACTED_DATA_PROMPT для валидации извлеченных данных, MERGE_CHUNKS_DATA_PROMPT для объединения данных из разных чанков с использованием накопленного контекста из предыдущих чанков (accumulated_context) для приоритизации и разрешения конфликтов, включая объединение услуг по названию
...
│   │   ├── ContractDetails.jsx      # Детальный просмотр контракта
│   │   ├── DataViewer.jsx            # Отображение JSON данных (renderField, renderResponsiblePersons, renderLocations, renderCounterparty, formatValue, translateField)
...
│   │   └── contractService.js       # Сервис для работы с контрактами (uploadContract, getContractStatus, getContractData, listContracts, deleteContract, getContractRawText, processContractForTests, testMCP1C)
│   │
│   ├── store/                        # Redux store
...
1. **MCP Server** (`server/mcp_server.py`) - обрабатывает SSE подключения и команды:
   ...
   - `_prepare_note` - формирует заметку для контрагента со всей информацией о контракте, включая услуги из спецификации/таблиц в конце заметки (название, количество, единица измерения, цена за единицу, общая стоимость)
   - `_update_counterparty` - обновляет данные существующего контрагента
   - `_create_agreement` - создает договор с контрагентом
   - `_attach_file` - прикрепляет файл контракта к контрагенту
...
2. **OData Client** (`client/oneс_client.py`) - взаимодействует с 1С через OData API:
   ...
   - `attach_file` - прикрепляет файлы к сущностям (базовая реализация)
...
5. **Agent Orchestrator** - выполняет pipeline:
   ...
   - Извлекает текст и таблицы из документа через docling (таблицы в markdown формате)
   - Проверяет размер документа: если документ большой (>64000 символов), разбивает на чанки через `DocumentProcessor.get_chunks_for_llm()`
   - Извлекает данные через `LLMService`: для больших документов обрабатывает каждый чанк отдельно. При обработке чанков накапливает полную информацию о договоре из предыдущих чанков в переменной `accumulated_data`, включая основную информацию о договоре (название, номер, дата, цена, НДС), описание услуг, услуги из спецификации/таблиц (название, количество, единица измерения, цена за единицу, общая стоимость), даты оказания услуг, условия оплаты, дополнительную информацию, информацию о контрагентах, ответственных лицах и адресах. Для каждого чанка после первого формирует контекстный блок через `_build_chunk_context()` со всей накопленной информацией и добавляет этот контекст в начало чанка перед отправкой в LLM. После обработки каждого чанка обновляет накопленные данные, приоритизируя более полные значения для строковых полей и объединяя уникальные значения для списков (ответственные лица, адреса, услуги по названию). Сохраняет контекст каждого чанка (первые 1000 символов текста) и полный накопленный контекст (accumulated_context) для каждого чанка. Затем выполняет финальную агрегацию через `LLMService.aggregate_chunks_data()` с передачей полного накопленного контекста для лучшего разрешения конфликтов между чанками через LLM. При ошибке агрегации используется fallback на `merge_extracted_data()`. Таблицы в markdown формате включены в контекст для LLM. Информация о всех запросах LLM (включая финальную агрегацию) сохраняется в `llm_requests` и в БД в таблице `processing_history`
   ...
   - Прикрепляет файл контракта к контрагенту
...
- **contract_data** - извлеченные данные контракта (inn, full_name, contract_price, dates, locations JSONB, responsible_persons JSONB, services JSONB - список услуг из спецификации/таблиц с полями: name, quantity, unit, unit_price, total_price, description)
- **processing_history** - история обработки контрактов
